...
// Writes file, standardizes newlines to LF, ensures trailing newline
function writeTemplateFile(filePath, content) {
    const standardizedContent = content.trimStart().replace(/\r\n/g, '\n') + '\n';
...
// --- Prisma Seeder Templates (UPDATED STRATEGY) ---
const prismaSeederTemplates = {
  ...
  "prisma": {
    ...
    // --- UPDATED PRISMA SCHEMA TEMPLATE ---
...
datasource db {
  ...
  // Make sure this matches the DATABASE_URL type in your .env file

  provider = "sqlite"     // DEFAULT: Simple setup, uses a file (see .env.example)
  // provider = "postgresql" // Uncomment for PostgreSQL
  // provider = "mysql"      // Uncomment for MySQL
  // provider = "sqlserver"   // Uncomment for SQL Server
  // provider = "mongodb"    // Uncomment for MongoDB

  url = env("DATABASE_URL") // This is loaded from the .env file
}
...
# Choose ONE and uncomment it, or add your own.
# Make sure the database provider you uncomment here matches the 'provider' uncommented in prisma/schema.prisma

# SQLite (DEFAULT: simplest local setup, uses a file in this project directory)
DATABASE_URL="file:./dev.db"

# PostgreSQL
# DATABASE_URL="postgresql://USER:PASSWORD@HOST:PORT/DATABASE?schema=public"
# Example: DATABASE_URL="postgresql://postgres:mysecretpassword@localhost:5432/prisma_dev?schema=public"

# MySQL
# DATABASE_URL="mysql://USER:PASSWORD@HOST:PORT/DATABASE"
# Example: DATABASE_URL="mysql://root:mysecretpassword@localhost:3306/prisma_dev"

# SQL Server
# DATABASE_URL="sqlserver://HOST:PORT;database=DATABASE;user=USER;password=PASSWORD;encrypt=true"
# Example: DATABASE_URL="sqlserver://localhost:1433;database=prisma_dev;user=sa;password=myStrong(!)Password;encrypt=false"

# MongoDB
# DATABASE_URL="mongodb+srv://USER:PASSWORD@HOST/DATABASE?retryWrites=true&w=majority"
# Example: DATABASE_URL="mongodb+srv://testuser:testpassword@cluster0.mongodb.net/prisma_dev?retryWrites=true&w=majority"

# --- Other Environment Variables (if needed) ---
# e.g., API keys, external service URLs
...
function printUsage() {
    ...
    logInfo('  prisma-seeder    Generates a basic Node.js/Prisma database seeder with multiple DB examples.');
    logInfo('');
...
try {
    ...
    if (command === 'mock-server') {
        ...
        logInfo(`Now configured with multi-database examples in '.env.example' and 'prisma/schema.prisma'.`);
        logInfo(`Defaults to SQLite for easy start.`);
        logInfo(`Next steps:`);
        logInfo(`  1. cd ${targetPath}`);
        logInfo(`  2. npm install`);
        logInfo(`  3. Configure DB: Edit 'prisma/schema.prisma' AND copy/edit '.env' to uncomment/set desired DB type & URL.`);
        logInfo(`  4. npx prisma migrate dev --name init (to setup database schema)`);
        logInfo(`  5. npm run prisma:seed (to run the seeder)`);

    } else {
        printUsage(); // Show usage if command is unknown
    ...
    // Don't print full usage on template errors, just the error message a user would expect.
    // printUsage();
